...
function dynamicData(propertiesArr) {
    ...
    for (var i = 0; i < propertiesArr.length; i++) {
        var address = propertiesArr[i]['Address_Line_1'] + " " + propertiesArr[i]['Address_Line_2'] + " " + propertiesArr[i]['Address_Line_3'] + " " + propertiesArr[i]['Locality'];
        var prodActive = propertiesArr[i]['Product_Active'] == true ? 'Active' : 'Sold Out';
...
function searchVal() {
    ...
    getAllProperties.forEach(element => {
        if ((element['Locality'].toLowerCase()).includes(userInput.toLowerCase())) {
...
function clicking(moduleName) {
    ...
    <div class="col-6 col-md-6">
        ...
        // var row = `
        //    <img src="../images/Map.webp" alt="map" style="width:100%;height:400px;">
        // `
